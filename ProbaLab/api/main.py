...
@app.post("/api/best-bets/backfill")
    ...
    try:
        ...
        def calc_stats(raw_bets):
            ...
            for key, legs in fun_groups.items():
                date, market = key
                all_win = all(l["result"] == "WIN" for l in legs)
                has_loss = any(l["result"] == "LOSS" for l in legs)
                is_void = all(l["result"] == "VOID" for l in legs)
...
def get_best_bets_history(
    ...
    try:
        ...
        for (d, m), legs in fun_groups.items():
            all_win = all(l.get("result") == "WIN" for l in legs)
            has_loss = any(l.get("result") == "LOSS" for l in legs)
            is_void = all(l.get("result") == "VOID" for l in legs)
            
            res = "PENDING"
            if has_loss: res = "LOSS"
            elif all_win: res = "WIN"
...
"""
    ...
    for pick in picks:
        try:
            ...
            if is_combine:
                # ── Combiné: ANY leg LOSS means combo LOSS ──────────────
                is_loss = False
                has_unknown = False
                details = []
                for sel in selections:
