...
def fetch_nhl_odds():
    ...
    for fix in fixtures:
        ...
        try:
            ...
            resp = requests.get(
                f"{HOCKEY_API_URL}/odds", headers=HEADERS, params={"game": api_id}, timeout=15
            )
            ...
            supabase.table("nhl_fixtures").update({"odds_json": odds_data}).eq(
                "api_fixture_id", api_id
            ).execute()

            logger.info(
                f"[NHL Odds] ✅ Cotes récupérées pour {fix['home_team']} vs {fix['away_team']}"
            )
            updated_count += 1
