...
from src.config import SEASON, logger, supabase
...
def evaluate_tickets() -> None:
    ...
    # 1. Picks non évalués
    unevaluated = supabase.table("ticket_picks").select("*").is_("is_won", "null").execute().data
    ...
    for pick in unevaluated:
        ...
        try:
            supabase.table("ticket_picks").update({"is_won": is_won, "evaluated_at": "now()"}).eq(
                "id", pick["id"]
            ).execute()
            evaluated_count += 1
...
def _print_ticket_stats() -> None:
    ...
    logger.info(f"\n{'=' * 60}")
    logger.info("  🎫 PERFORMANCE DES TICKETS")
    logger.info(f"{'=' * 60}")
    ...
    for ticket_type in ["SAFE", "FUN", "JACKPOT"]:
        ...
        if completed_tickets > 0:
            ...
            logger.info(f"    Tickets: {won_tickets}/{completed_tickets} gagnés ({ticket_pct}%)")
