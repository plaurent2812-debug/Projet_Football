...
def run():
    ...
    data = supabase.table("fixtures").select("id, api_fixture_id, home_team, away_team, events_json").neq("status", "NS").order("date", desc=True).limit(150).execute()
    ...
    for fix in fixtures:
        ...
        logger.info(f"Updating events for {fix.get('home_team')} vs {fix.get('away_team')} (ID: {fix['api_fixture_id']})")
        ...
        supabase.table("fixtures").update({"events_json": goals_list}).eq("api_fixture_id", api_fid).execute()
        updated_count += 1
