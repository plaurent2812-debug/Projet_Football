...
def fetch_boxscore(api_game_id: str) -> Optional[dict]:
...
def evaluate_nhl_predictions(days_back=3):
    ...
    for fix in fixtures:
        ...
        def add_eval(player_dict, stat_name, pari_name, achieved_key, expected_val=1):
            ...
            ml_prob = player_dict.get(f"ml_prob_{stat_name}", player_dict.get(f"prob_{stat_name}", 0))
