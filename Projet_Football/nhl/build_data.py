...
def build_nhl_dataset(output_file="nhl_dataset.csv"):
    ...
    response = supabase.table("nhl_fixtures").select(
        "api_fixture_id, date, home_team, away_team, stats_json"
    ).in_("status", ["Final", "FINAL", "FT", "OFF"]).not_.is_("stats_json", "null").execute()
    ...
    for idx, fix in enumerate(fixtures):
        ...
        for pid, player_features in unique_players.items():
            ...
            if perf is not None:
                ...
                row["label_shot"] = 1 if perf["shots"] >= 3 else 0 # Threshold standard NHL: Over 2.5
