...
def process_daily_reflection(sport="football"):
    ...
    for row in failed_preds:
        ...
        logger.info(f"[Reflection] Analyse de l'échec sur {match_name} (Confiance: {row['pred_confidence']}/10)")
...
"""
        ...
        if ai_json and "learning_text" in ai_json:
            ...
            try:
             ...
             logger.warning(f"   ⚠️ Gemini n'a pas retourné de JSON valide pour ce match.")
                
    logger.info(f"[Reflection] Terminé ! {new_learnings} nouvelles règles ajoutées à la mémoire du modèle.")
